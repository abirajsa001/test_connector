# connect-payment-integration-novalnet
This repository provides a [connect](https://docs.commercetools.com/connect) for integration to Novalnet payment service provider (PSP).
...
## Overview
The novalnet-integration connector contains two modules :  
- Enabler: Acts as a wrapper implementation in which frontend components from Novalnet embedded. It gives control to checkout product on when and how to load the connector frontend based on business configuration. In cases connector is used directly and not through Checkout product, the connector library can be loaded directly on frontend than the PSP one.
- Processor : Acts as backend services which is middleware to integrate with Novalnet platform. It is mainly responsible for managing transactions with Novalnet and updating payment entity in composable commerce.  `connect-payment-sdk` will be offered to be used in connector to manage request context, sessions and other tools necessary to transact.
...
flowchart TD
    ...
    processor--"5. Request payment \noperations"-->psp("Novalnet payment service provider")
    psp("Novalnet payment service provider")--"6. Return response"-->processor
...
4. The processor transforms the received request and send them to Novalnet platform.
5. Novalnet processes the payment operations and return the response to processor.
...
#### 3. Novalnet account credentials
Various account data provided by Novalnet are necessary to be configured so that the requests from the connect application can be authenticated by Novalnet platform within the integration.
Their values are taken as input as environment variables/ configuration for connect with variable names `NOVALNET_ACCESS_KEY`, `NOVALNET_PAYMENT_ACCESS_KEY`, `NOVALNET_VENDOR_ID`, `NOVALNET_AUTH_CODE`, `NOVALNET_PRODUCT_ID` and `NOVALNET_ENVIRONMENT`.

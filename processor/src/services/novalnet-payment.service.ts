...
export class NovalnetPaymentService extends AbstractPaymentService {
  ...
  public async createPayment(opts: { data: CreatePaymentRequestDTO }): Promise<CreatePaymentResponseDTO> {
    ...
    if (!ctCart.billingAddress?.firstName || !ctCart.billingAddress?.lastName || !ctCart.billingAddress?.email) {
      throw new ErrorInvalidOperation('Missing required billing address information');
    }
  ...
  async reversePayment(request: ReversePaymentRequest): Promise<PaymentProviderModificationResponse> {
    ...
    const hasCharge = await this.ctPaymentService.hasTransactionInState({ 
      payment: request.payment, 
      transactionType: 'Charge', 
      states: ['Success'] 
    });
    const hasAuthorization = await this.ctPaymentService.hasTransactionInState({ 
      payment: request.payment, 
